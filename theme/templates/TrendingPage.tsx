    ...
    <main className="px-4 pt-6 pb-16 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-6xl flex-col gap-6 sm:gap-8">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 sm:text-3xl">Трендовые товары</h1>
            <p className="text-sm text-gray-500">Топ по просмотрам за последние 30 дней</p>
          </div>
        ...
        {products.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
              {visible.map((product) => (
                <ProductCart key={`trending-${String(product.id)}`} product={product}/>
              ))}
            </div>
